import { Component, OnInit } from '@angular/core';
import { ServerService } from '../../core/services/server.service';
import { Menu } from '../../model/menu.model';
import { MasterPageService } from '../../core/services/master-page.service';
import { LanguageService } from '../../core/services/language.service';
import { MenuItem } from '../../model/menu-item.model';
import { UserService } from '../../core/services/user.service';

@Component({
  selector: 'app-navigation-bar',
  templateUrl: './navigation-bar.component.html',
  styleUrls: ['./navigation-bar.component.scss']
})
export class NavigationBarComponent implements OnInit {

  // tslint:disable-next-line:max-line-length
  // HEADER_LOGO: String = 'data:image/png;base64,';

  listMenu: Menu[] = [];
  compLanguage = this.language.currentLanguage.compNavigationBar;

  // Data Menu
  menuHome: Menu = new Menu(this.compLanguage.menuHome, 'assets/menu-icon/icon-home.png', 'home', 1);
  menuVNRegions: Menu = new Menu(this.compLanguage.menuVNRegions, 'assets/menu-icon/icon-region.png', '', 2);
  menuTravel: Menu = new Menu(this.compLanguage.menuTravel, 'assets/menu-icon/icon-travel.png', '', 3);
  menuCuisine: Menu = new Menu(this.compLanguage.menuCuisine, 'assets/menu-icon/icon-cuisine.png', '', 4);
  menuPlans: Menu = new Menu(this.compLanguage.menuPlans, 'assets/menu-icon/icon-plans.png', '', 5);
  menuTrend: Menu = new Menu(this.compLanguage.menuTrend, 'assets/menu-icon/icon-trend.png', 'trend', 6);
  // menuPersonal: Menu = new Menu(this.compLanguage.menuPersonal, 'assets/menu-icon/icon-user.png', '', 7);
  menuFilter: Menu = new Menu(this.compLanguage.menuFilter, 'assets/menu-icon/icons8-slider-64.png', 'filter/all', 7);
  menuAbout: Menu = new Menu(this.compLanguage.menuAbout, 'assets/menu-icon/icon-about.png', '', 8);

  // Menu Item Regions
  menuItemTheNorth: MenuItem = new MenuItem(this.compLanguage.menuItemTheNorth, 'region/north');
  menuItemTheCentral: MenuItem = new MenuItem(this.compLanguage.menuItemTheCentral, 'region/central');
  menuItemTheSouth: MenuItem = new MenuItem(this.compLanguage.menuItemTheSouth, 'region/south');

  // Menu Item Travel
  menuItemHotTravel: MenuItem = new MenuItem(this.compLanguage.menuItemHot, 'filter/travel/hot');
  menuItemRecentTravel: MenuItem = new MenuItem(this.compLanguage.menuItemMostRecent, 'filter/travel/recent');

  // Menu Item Cuisine
  menuItemHotCuisine: MenuItem = new MenuItem(this.compLanguage.menuItemHot, 'filter/cuisine/hot');
  menuItemRecentCuisine: MenuItem = new MenuItem(this.compLanguage.menuItemMostRecent, 'filter/cuisine/recent');

  // Menu Item Plans
  menuItemOneDay: MenuItem = new MenuItem(this.compLanguage.menuItemOneDay, '');
  menuItemOneWeek: MenuItem = new MenuItem(this.compLanguage.menuItemOneWeek, '');
  menuItemMoreThanAWeek: MenuItem = new MenuItem(this.compLanguage.menuItemMoreThanAWeek, '');

  // Menu Item Personal Information
  // menuItemPersonalInfo: MenuItem = new MenuItem(this.compLanguage.menuItemProfile, 'user/profile');
  // menuItemChangePassword: MenuItem = new MenuItem(this.compLanguage.menuItemChangePassword, '');
  // menuItemContribution: MenuItem = new MenuItem(this.compLanguage.menuItemContribution, '');

  // Menu Filter

  // Menu Item About Us
  menuItemPolicies: MenuItem = new MenuItem(this.compLanguage.menuItemPolicies, 'additional/policies');
  menuItemFeedback: MenuItem = new MenuItem(this.compLanguage.menuItemFeedback, 'additional/feedback');

  constructor(private server: ServerService, private masterPage: MasterPageService,
    private language: LanguageService, private user: UserService) { }

  ngOnInit() {
    this.compLanguage = this.language.currentLanguage.compNavigationBar;
    this.language.hasChangeLanguage.subscribe(() => this.compLanguage = this.language.currentLanguage.compNavigationBar);

    this.setMenu();

    this.language.hasChangeLanguage.subscribe(() => {
      this.refreshLanguage();
      this.compLanguage = this.language.currentLanguage.compNavigationBar;
    });
  }

  refreshLanguage() {
    this.menuHome.name = this.compLanguage.menuHome;
    this.menuVNRegions.name = this.compLanguage.menuVNRegions;
    this.menuTravel.name = this.compLanguage.menuTravel;
    this.menuCuisine.name = this.compLanguage.menuCuisine;
    this.menuPlans.name = this.compLanguage.menuPlans;
    this.menuTrend.name = this.compLanguage.menuTrend;
    this.menuFilter.name = this.compLanguage.menuFilter;
    // this.menuPersonal.name = this.compLanguage.menuPersonal;
    this.menuAbout.name = this.compLanguage.menuAbout;
    this.menuItemTheNorth.name = this.compLanguage.menuItemTheNorth;
    this.menuItemTheCentral.name = this.compLanguage.menuItemTheCentral;
    this.menuItemTheSouth.name = this.compLanguage.menuItemTheSouth;
    this.menuItemHotTravel.name = this.compLanguage.menuItemHot;
    this.menuItemRecentTravel.name = this.compLanguage.menuItemMostRecent;
    this.menuItemHotCuisine.name = this.compLanguage.menuItemHot;
    this.menuItemRecentCuisine.name = this.compLanguage.menuItemMostRecent;
    this.menuItemOneDay.name = this.compLanguage.menuItemOneDay;
    this.menuItemOneWeek.name = this.compLanguage.menuItemOneWeek;
    this.menuItemMoreThanAWeek.name = this.compLanguage.menuItemMoreThanAWeek;
    // this.menuItemPersonalInfo.name = this.compLanguage.menuItemProfile;
    // this.menuItemChangePassword.name = this.compLanguage.menuItemChangePassword;
    // this.menuItemContribution.name = this.compLanguage.menuItemContribution;
    this.menuItemPolicies.name = this.compLanguage.menuItemPolicies;
    this.menuItemFeedback.name = this.compLanguage.menuItemFeedback;
  }

  setMenu() {
    this.listMenu = [];
    this.setMenuItemToMenu();
    this.listMenu.push(
      this.menuHome,
      this.menuVNRegions,
      this.menuTravel,
      this.menuCuisine,
      // this.menuPlans,
      this.menuFilter,
      this.menuTrend,
      // this.menuPersonal,
      this.menuAbout
    );
  }

  setMenuItemToMenu() {
    this.menuVNRegions.listItem.push(this.menuItemTheNorth, this.menuItemTheCentral, this.menuItemTheSouth);
    this.menuTravel.listItem.push(this.menuItemHotTravel, this.menuItemRecentTravel);
    this.menuCuisine.listItem.push(this.menuItemHotCuisine, this.menuItemRecentCuisine);
    this.menuPlans.listItem.push(this.menuItemOneDay, this.menuItemOneWeek, this.menuItemMoreThanAWeek);
    // this.menuPersonal.listItem.push(this.menuItemPersonalInfo, this.menuItemChangePassword, this.menuItemContribution);
    this.menuAbout.listItem.push(this.menuItemPolicies, this.menuItemFeedback);
  }

}
